...
const fetchAvailableStock = async (productId: string): Promise<number> => {
  ...
  for (const row of data ?? []) {
    ...
    const type = (row as any).stock_movements?.type as
      | "IN"
      | "OUT"
      | "ADJUST"
      | undefined;
...
const SalesPage = () => {
  ...
  const handleAddByCode = async (codeValue?: string) => {
    ...
    try {
      ...
      if (available <= inCart) {
        toast.error(
          `Нема доволно залиха. Достапно: ${available}, во кошничка: ${inCart}.`,
        );
        return;
      }
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Интерна продажба со баркод читач. Додај артикли во кошничка и зачувај.
            </p>
          </div>
      ...
      <div className="rounded-2xl bg-white p-4 md:p-6 shadow-sm border border-slate-200 space-y-4">
        ...
        {cart.length === 0 ? (
          <div className="text-sm text-slate-500">
            Нема додадени артикли. Скенирај баркод.
          </div>
        ) : (
            ...
            <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-slate-800">
                  {totals.subtotal.toFixed(2)} ден.
                </span>
              </div>
              <div className="flex items-center justify-between text-sm mt-2">
                <span className="text-slate-600">Попуст</span>
                <span className="font-semibold text-slate-800">
                  -{totals.discountTotal.toFixed(2)} ден.
                </span>
              </div>
