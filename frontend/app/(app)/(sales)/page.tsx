...
const fetchProductByCode = async (
	...
	return (await api.get<ProductStockRow | null>(
...
const SalesPage = () => {
	...
	useEffect(() => {
		const handler = (e: KeyboardEvent) => {
			if (e.key === 'F4') { e.preventDefault(); searchInputRef.current?.focus(); searchInputRef.current?.select(); }
			if (e.key === 'F9') { e.preventDefault(); void handleSubmitSale(); }
		};
	...
	const resetSale = () => {
		setCode(''); setNote(''); resetCart(); setScanError(null);
		setSuggestions([]); setSuggestOpen(false); setFocusProductId(null);
		setPaymentMethod('CASH'); setCashReceivedStr('');
	};
	...
	const handleAddByCode = async (codeValue?: string) => {
		...
		if (!value) { toast.error('Внеси баркод или PLU.'); return; }
		setBusy(true);
		try {
			const row = await fetchProductByCode(value, storeNo);
			if (!row) { toast.error(`Не е пронајден производ во продавница ${storeNo}.`); return; }
			await addProductToCart(row);
			setCode(''); setSuggestions([]); setSuggestOpen(false);
		} catch (e) { console.error(e); toast.error('Грешка при барање на производ.'); }
	...
	const handleSubmitSale = async () => {
		...
		try {
			...
			if (msg !== 'empty-cart' && msg !== 'insufficient-cash') { console.error(e); toast.error('Грешка при зачувување на продажбата.'); }
		} finally { setBusy(false); }
	};
	...
	const handleScan = (detected: IDetectedBarcode[]) => {
		...
		if (!raw) return;
		setScannerOpen(false); setScanError(null);
		setCode(raw); void handleAddByCode(raw);
	};
	...
	return (
		<div className="flex flex-col h-full min-h-0 gap-3">
			...
			<div className="card px-4 py-3 shrink-0">
				<div className="flex items-center gap-3 flex-wrap">
					...
					<div className="flex items-center gap-2 shrink-0">
						...
						<select
							value={storeNo}
							onChange={(e) => { setStoreNo(Number(e.target.value) as 20 | 30); setCode(''); setSuggestions([]); setSuggestOpen(false); }}
						...
						<div className="flex items-center gap-2">
							<div className="relative flex-1">
								...
								<input
									...
									onFocus={() => { if (code.trim().length > 0) setSuggestOpen(true); }}
									onKeyDown={(e) => {
										if (e.key === 'Enter') { e.preventDefault(); void handleAddByCode(); }
											...
											<div className="flex items-center gap-3">
												...
												<div className="flex-1 min-w-0">
													...
													<div className="text-[11px] text-slate-500">
														...
														{s.barcode ? <> · <span className="font-medium text-slate-700">{s.barcode}</span></> : null}
													</div>
												</div>
												<div className="shrink-0 text-right">
													<div className="text-sm font-bold text-slate-900 tabular-nums">{num(s.selling_price).toFixed(2)} ден.</div>
													<div className="text-[11px] text-slate-500">
														Зал: <span className={`font-semibold ${num(s.qty_on_hand) <= 0 ? 'text-red-600' : num(s.qty_on_hand) <= 3 ? 'text-amber-600' : 'text-emerald-700'}`}>{num(s.qty_on_hand)}</span>
													</div>
												</div>
											</div>
										</button>
					...
					<div className="hidden lg:flex items-center gap-2 shrink-0 text-[11px] text-slate-400">
						<span className="kbd">F4</span><span>Пребарај</span>
					...
					<div className="card px-4 py-3 shrink-0">
						<div className="flex items-center justify-between mb-2.5">
							<div className="flex items-center gap-2">
								{recentProducts.length > 0
									? <><FiClock className="w-3.5 h-3.5 text-slate-400" /><span className="text-xs font-semibold text-slate-600">Последно користени</span></>
									: <><FiZap className="w-3.5 h-3.5 text-slate-400" /><span className="text-xs font-semibold text-slate-600">Брзи производи</span></>
								}
							</div>
							{recentProducts.length > 0 && (
								<button
									type="button"
									onClick={() => { localStorage.removeItem(RECENT_KEY); setRecentProducts([]); }}
						...
						{recentProducts.length === 0 ? (
							<div className="py-3 text-center text-xs text-slate-400">
								Производите кои ги додадете ќе се прикажат тука за брз пристап.
							</div>
						) : (
								...
								{recentProducts.map((p) => {
									...
									return (
										...
										>
											...
											<div className={`absolute top-1.5 right-1.5 h-1.5 w-1.5 rounded-full ${isZero ? 'bg-red-400' : isLow ? 'bg-amber-400' : 'bg-emerald-400'}`} />
										</button>
									);
								})}
							</div>
								...
								<div className="space-y-2">
									{cart.map((item) => (
										<CartItemCard
											...
											onRemove={() => { setFocusProductId(null); removeItem(item.product.id); }}
											onQtyChange={(q) => { setFocusProductId(null); changeQty(item.product.id, q); }}
											onFinalPriceChange={(raw) => { setFocusProductId(null); patchFinalPrice(item.product.id, raw); }}
			...
			<ScannerModal
				...
				onScan={handleScan}
				onError={(err) => { console.error(err); setScanError('Грешка при пристап до камерата.'); }}
			/>
		</div>
	);
};
