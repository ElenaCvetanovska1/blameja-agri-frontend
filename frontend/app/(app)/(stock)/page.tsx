...
export default function StockPage() {
	...
	const stats = useMemo(() => {
		...
		const lowStock = rows.filter((r) => { const q = num(r.qty_on_hand); return q > 0 && q <= 3; }).length;
	...
	const visibleRows = useMemo(() => {
		...
		list.sort((a, b) => {
			...
			if (sortKey === 'PLU') { const pa = pluAsNumber(a.plu) ?? -1; const pb = pluAsNumber(b.plu) ?? -1; return dir * (pa - pb); }
		...
		<div className="flex flex-col h-full min-h-0 gap-3">
			...
			<div className="grid grid-cols-3 gap-3 shrink-0">
				...
				<div className={`card px-3 py-2.5 flex items-center gap-2.5 ${stats.lowStock > 0 ? 'border-amber-200 bg-amber-50/40' : ''}`}>
					<div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${stats.lowStock > 0 ? 'bg-amber-100' : 'bg-slate-100'}`}>
					...
					<div>
						...
						<div className={`text-xl font-extrabold tabular-nums leading-tight ${stats.lowStock > 0 ? 'text-amber-700' : 'text-slate-900'}`}>{stats.lowStock}</div>
					</div>
				</div>

				<div className={`card px-3 py-2.5 flex items-center gap-2.5 ${stats.outOfStock > 0 ? 'border-red-200 bg-red-50/30' : ''}`}>
					<div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${stats.outOfStock > 0 ? 'bg-red-100' : 'bg-slate-100'}`}>
					...
					<div>
						...
						<div className={`text-xl font-extrabold tabular-nums leading-tight ${stats.outOfStock > 0 ? 'text-red-600' : 'text-slate-900'}`}>{stats.outOfStock}</div>
					</div>
				</div>
			</div>
			...
			<div className="card px-4 py-3 shrink-0">
				<div className="flex flex-col sm:flex-row gap-2.5 items-end">
					<div className="flex-1 min-w-0">
						<label className="form-label flex items-center gap-1" htmlFor="stock-search">
							<FiSearch className="w-3 h-3" />Пребарај
						</label>
					...
					<div className="w-full sm:w-40">
						<label className="form-label flex items-center gap-1" htmlFor="stock-category-filter">
							<FiFilter className="w-3 h-3" />Категорија
						</label>
						<select id="stock-category-filter" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="form-input">
					...
					<div className="w-full sm:w-32">
						<label className="form-label" htmlFor="stock-sort-key">Сортирај по</label>
						<select id="stock-sort-key" value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)} className="form-input">
					...
					<div>
						...
						>
							{sortDir === 'ASC'
								? <><FiArrowUp className="w-4 h-4 text-blamejaGreen" /><span className="hidden sm:inline text-xs">Растечки</span></>
								: <><FiArrowDown className="w-4 h-4 text-blamejaOrange" /><span className="hidden sm:inline text-xs">Опаѓачки</span></>
							}
						</button>
					</div>
				</div>
				...
				{(search || categoryFilter) && (
					<div className="mt-2 flex items-center gap-2 text-xs text-slate-500">
						...
						{search && <span className="inline-flex items-center gap-1 rounded-full bg-blamejaGreenSoft px-2 py-0.5 text-blamejaGreenDark font-medium">„{search}" <button type="button" onClick={() => setSearch('')} className="hover:text-red-500">×</button></span>}
						{categoryFilter && <span className="inline-flex items-center gap-1 rounded-full bg-blamejaGreenSoft px-2 py-0.5 text-blamejaGreenDark font-medium">{categoryFilter} <button type="button" onClick={() => setCategoryFilter('')} className="hover:text-red-500">×</button></span>}
			...
			{selected && <StockAdjustModal open={!!selected} row={selected} onClose={() => setSelected(null)} />}
			{toDelete && <DeleteProductModal open={!!toDelete} row={toDelete} onClose={() => setToDelete(null)} />}
		</div>
	);
}
